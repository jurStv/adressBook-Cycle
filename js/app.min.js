...
var aLen=aChildren.length;var bLen=bChildren.length;var len=aLen>bLen?aLen:bLen;for(var i=0;i<len;i++){var leftNode=aChildren[i];var rightNode=bChildren[i];index+=1;if(!leftNode){if(rightNode){apply=appendPatch(apply,new VPatch(VPatch.INSERT,null,rightNode))}}else{walk(leftNode,rightNode,patch,index)}if(isVNode(leftNode)&&leftNode.count){index+=leftNode.count}}if(orderedSet.moves){apply=appendPatch(apply,new VPatch(VPatch.ORDER,a,orderedSet.moves))}return apply}function clearState(vNode,patch,index){unhook(vNode,patch,index);destroyWidgets(vNode,patch,index)}function destroyWidgets(vNode,patch,index){if(isWidget(vNode)){if(typeof vNode.destroy==="function"){patch[index]=appendPatch(patch[index],new VPatch(VPatch.REMOVE,vNode,null))}}else if(isVNode(vNode)&&(vNode.hasWidgets||vNode.hasThunks)){var children=vNode.children;var len=children.length;for(var i=0;i<len;i++){var child=children[i];index+=1;destroyWidgets(child,patch,index);if(isVNode(child)&&child.count){index+=child.count}}}else if(isThunk(vNode)){thunks(vNode,null,patch,index)}}function thunks(a,b,patch,index){var nodes=handleThunk(a,b);var thunkPatch=diff(nodes.a,nodes.b);if(hasPatches(thunkPatch)){patch[index]=new VPatch(VPatch.THUNK,null,thunkPatch)}}function hasPatches(patch){for(var index in patch){if(index!=="a"){return true}}return false}function unhook(vNode,patch,index){if(isVNode(vNode)){if(vNode.hooks){patch[index]=appendPatch(patch[index],new VPatch(VPatch.PROPS,vNode,undefinedKeys(vNode.hooks)))}if(vNode.descendantHooks||vNode.hasThunks){var children=vNode.children;var len=children.length;for(var i=0;i<len;i++){var child=children[i];index+=1;unhook(child,patch,index);if(isVNode(child)&&child.count){index+=child.count}}}}else if(isThunk(vNode)){thunks(vNode,null,patch,index)}}function undefinedKeys(obj){var result={};for(var key in obj){result[key]=undefined}return result}function reorder(aChildren,bChildren){var bChildIndex=keyIndex(bChildren);var bKeys=bChildIndex.keys;var bFree=bChildIndex.free;if(bFree.length===bChildren.length){return{children:bChildren,moves:null}}var aChildIndex=keyIndex(aChildren);var aKeys=aChildIndex.keys;var aFree=aChildIndex.free;if(aFree.length===aChildren.length){return{children:bChildren,moves:null}}var newChildren=[];var freeIndex=0;var freeCount=bFree.length;var deletedItems=0;for(var i=0;i<aChildren.length;i++){var aItem=aChildren[i];var itemIndex;if(aItem.key){if(bKeys.hasOwnProperty(aItem.key)){itemIndex=bKeys[aItem.key];newChildren.push(bChildren[itemIndex])}else{itemIndex=i-deletedItems++;newChildren.push(null)}}else{if(freeIndex<freeCount){itemIndex=bFree[freeIndex++];newChildren.push(bChildren[itemIndex])}else{itemIndex=i-deletedItems++;newChildren.push(null)}}}var lastFreeIndex=freeIndex>=bFree.length?bChildren.length:bFree[freeIndex];for(var j=0;j<bChildren.length;j++){var newItem=bChildren[j];if(newItem.key){if(!aKeys.hasOwnProperty(newItem.key)){newChildren.push(newItem)}}else if(j>=lastFreeIndex){newChildren.push(newItem)}}var simulate=newChildren.slice();var simulateIndex=0;var removes=[];var inserts=[];var simulateItem;for(var k=0;k<bChildren.length;){var wantedItem=bChildren[k];simulateItem=simulate[simulateIndex];while(simulateItem===null&&simulate.length){removes.push(remove(simulate,simulateIndex,null));simulateItem=simulate[simulateIndex]}if(!simulateItem||simulateItem.key!==wantedItem.key){if(wantedItem.key){if(simulateItem&&simulateItem.key){if(bKeys[simulateItem.key]!==k+1){removes.push(remove(simulate,simulateIndex,simulateItem.key));simulateItem=simulate[simulateIndex];if(!simulateItem||simulateItem.key!==wantedItem.key){inserts.push({key:wantedItem.key,to:k})}else{simulateIndex++}}else{inserts.push({key:wantedItem.key,to:k})}}else{inserts.push({key:wantedItem.key,to:k})}k++}else if(simulateItem&&simulateItem.key){removes.push(remove(simulate,simulateIndex,simulateItem.key))}}else{simulateIndex++;k++}}while(simulateIndex<simulate.length){simulateItem=simulate[simulateIndex];removes.push(remove(simulate,simulateIndex,simulateItem&&simulateItem.key))}if(removes.length===deletedItems&&!inserts.length){return{children:newChildren,moves:null}}return{children:newChildren,moves:{removes:removes,inserts:inserts}}}function remove(arr,index,key){arr.splice(index,1);return{from:index,key:key}}function keyIndex(children){var keys={};var free=[];var length=children.length;for(var i=0;i<length;i++){var child=children[i];if(child.key){keys[child.key]=i}else{free.push(i)}}return{keys:keys,free:free}}function appendPatch(apply,patch){if(apply){if(isArray(apply)){apply.push(patch)}else{apply=[apply,patch]}return apply}else{return patch}}},{"../vnode/handle-thunk":101,"../vnode/is-thunk":102,"../vnode/is-vnode":104,"../vnode/is-vtext":105,"../vnode/is-widget":106,"../vnode/vpatch":109,"./diff-props":111,"x-is-array":113}],113:[function(require,module,exports){var nativeIsArray=Array.isArray;var toString=Object.prototype.toString;module.exports=nativeIsArray||isArray;function isArray(obj){return toString.call(obj)==="[object Array]"}},{}],114:[function(require,module,exports){},{}],115:[function(require,module,exports){var process=module.exports={};var queue=[];var draining=false;var currentQueue;var queueIndex=-1;function cleanUpNextTick(){draining=false;if(currentQueue.length){queue=currentQueue.concat(queue)}else{queueIndex=-1}if(queue.length){drainQueue()}}function drainQueue(){if(draining){return}var timeout=setTimeout(cleanUpNextTick);draining=true;var len=queue.length;while(len){currentQueue=queue;queue=[];while(++queueIndex<len){currentQueue[queueIndex].run()}queueIndex=-1;len=queue.length}currentQueue=null;draining=false;clearTimeout(timeout)}process.nextTick=function(fun){var args=new Array(arguments.length-1);if(arguments.length>1){for(var i=1;i<arguments.length;i++){args[i-1]=arguments[i]}}queue.push(new Item(fun,args));if(queue.length===1&&!draining){setTimeout(drainQueue,0)}};function Item(fun,array){this.fun=fun;this.array=array}Item.prototype.run=function(){this.fun.apply(null,this.array)};process.title="browser";process.browser=true;process.env={};process.argv=[];process.version="";process.versions={};function noop(){}process.on=noop;process.addListener=noop;process.once=noop;process.off=noop;process.removeListener=noop;process.removeAllListeners=noop;process.emit=noop;process.binding=function(name){throw new Error("process.binding is not supported")};process.cwd=function(){return"/"};process.chdir=function(dir){throw new Error("process.chdir is not supported")};process.umask=function(){return 0}},{}],116:[function(require,module,exports){(function(app){"use strict";var namespace="cuid",c=0,blockSize=4,base=36,discreteValues=Math.pow(base,blockSize),pad=function pad(num,size){var s="000000000"+num;return s.substr(s.length-size)},randomBlock=function randomBlock(){return pad((Math.random()*discreteValues<<0).toString(base),blockSize)},safeCounter=function(){c=c<discreteValues?c:0;c++;return c-1},api=function cuid(){var letter="c",timestamp=(new Date).getTime().toString(base),counter,fingerprint=api.fingerprint(),random=randomBlock()+randomBlock();counter=pad(safeCounter().toString(base),blockSize);return letter+timestamp+counter+fingerprint+random};api.slug=function slug(){var date=(new Date).getTime().toString(36),counter,print=api.fingerprint().slice(0,1)+api.fingerprint().slice(-1),random=randomBlock().slice(-2);counter=safeCounter().toString(36).slice(-4);return date.slice(-2)+counter+print+random};api.globalCount=function globalCount(){var cache=function calc(){var i,count=0;for(i in window){count++}return count}();api.globalCount=function(){return cache};return cache};api.fingerprint=function browserPrint(){return pad((navigator.mimeTypes.length+navigator.userAgent.length).toString(36)+api.globalCount().toString(36),4)};if(app.register){app.register(namespace,api)}else if(typeof module!=="undefined"){module.exports=api}else{app[namespace]=api}})(this.applitude||this)},{}],117:[function(require,module,exports){var _concat=require("./internal/_concat");var _curry2=require("./internal/_curry2");module.exports=_curry2(function append(el,list){return _concat(list,[el])})},{"./internal/_concat":119,"./internal/_curry2":122}],118:[function(require,module,exports){var _containsWith=require("./internal/_containsWith");var _curry3=require("./internal/_curry3");module.exports=_curry3(_containsWith)},{"./internal/_containsWith":120,"./internal/_curry3":123}],119:[function(require,module,exports){module.exports=function _concat(set1,set2){set1=set1||[];set2=set2||[];var idx;var len1=set1.length;var len2=set2.length;var result=[];idx=0;while(idx<len1){result[result.length]=set1[idx];idx+=1}idx=0;while(idx<len2){result[result.length]=set2[idx];idx+=1}return result}},{}],120:[function(require,module,exports){module.exports=function _containsWith(pred,x,list){var idx=0,len=list.length;while(idx<len){if(pred(x,list[idx])){return true}idx+=1}return false}},{}],121:[function(require,module,exports){module.exports=function _curry1(fn){return function f1(a){if(arguments.length===0){return f1}else if(a!=null&&a["@@functional/placeholder"]===true){return f1}else{return fn.apply(this,arguments)}}}},{}],122:[function(require,module,exports){var _curry1=require("./_curry1");module.exports=function _curry2(fn){return function f2(a,b){var n=arguments.length;if(n===0){return f2}else if(n===1&&a!=null&&a["@@functional/placeholder"]===true){return f2}else if(n===1){return _curry1(function(b){return fn(a,b)})}else if(n===2&&a!=null&&a["@@functional/placeholder"]===true&&b!=null&&b["@@functional/placeholder"]===true){return f2}else if(n===2&&a!=null&&a["@@functional/placeholder"]===true){return _curry1(function(a){return fn(a,b)})}else if(n===2&&b!=null&&b["@@functional/placeholder"]===true){return _curry1(function(b){return fn(a,b)})}else{return fn(a,b)}}}},{"./_curry1":121}],123:[function(require,module,exports){var _curry1=require("./_curry1");var _curry2=require("./_curry2");module.exports=function _curry3(fn){return function f3(a,b,c){var n=arguments.length;if(n===0){return f3}else if(n===1&&a!=null&&a["@@functional/placeholder"]===true){return f3}else if(n===1){return _curry2(function(b,c){return fn(a,b,c)})}else if(n===2&&a!=null&&a["@@functional/placeholder"]===true&&b!=null&&b["@@functional/placeholder"]===true){return f3}else if(n===2&&a!=null&&a["@@functional/placeholder"]===true){return _curry2(function(a,c){return fn(a,b,c)})}else if(n===2&&b!=null&&b["@@functional/placeholder"]===true){return _curry2(function(b,c){return fn(a,b,c)})}else if(n===2){return _curry1(function(c){return fn(a,b,c)})}else if(n===3&&a!=null&&a["@@functional/placeholder"]===true&&b!=null&&b["@@functional/placeholder"]===true&&c!=null&&c["@@functional/placeholder"]===true){return f3}else if(n===3&&a!=null&&a["@@functional/placeholder"]===true&&b!=null&&b["@@functional/placeholder"]===true){return _curry2(function(a,b){return fn(a,b,c)})}else if(n===3&&a!=null&&a["@@functional/placeholder"]===true&&c!=null&&c["@@functional/placeholder"]===true){return _curry2(function(a,c){return fn(a,b,c)})}else if(n===3&&b!=null&&b["@@functional/placeholder"]===true&&c!=null&&c["@@functional/placeholder"]===true){return _curry2(function(b,c){return fn(a,b,c)})}else if(n===3&&a!=null&&a["@@functional/placeholder"]===true){return _curry1(function(a){return fn(a,b,c)})}else if(n===3&&b!=null&&b["@@functional/placeholder"]===true){return _curry1(function(b){return fn(a,b,c)})}else if(n===3&&c!=null&&c["@@functional/placeholder"]===true){return _curry1(function(c){return fn(a,b,c)})}else{return fn(a,b,c)}}}},{"./_curry1":121,"./_curry2":122}],124:[function(require,module,exports){"use strict";function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{"default":obj}}var _cycleCore=require("@cycle/core");var _cycleDom=require("@cycle/dom");var _componentsAdresses=require("./components/adresses");var _componentsAdresses2=_interopRequireDefault(_componentsAdresses);var _drivers=require("./drivers");var _drivers2=_interopRequireDefault(_drivers);var main=_componentsAdresses2["default"];(0,_cycleCore.run)(main,{DOM:(0,_cycleDom.makeDOMDriver)("#app"),localStorageSource:_drivers2["default"].makeLocalStorageSourceDriver("adress-book"),localStorageSink:_drivers2["default"].makeLocalStorageSinkDriver("adress-book")})},{"./components/adresses":126,"./drivers":134,"@cycle/core":1,"@cycle/dom":5}],125:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{"default":obj}}var _cycleCore=require("@cycle/core");var _cycleDom=require("@cycle/dom");var _formValidator=require("../form-validator");var _formValidator2=_interopRequireDefault(_formValidator);var _cuid=require("cuid");var _cuid2=_interopRequireDefault(_cuid);var formValidator=new _formValidator2["default"]({firstname:/^\S+/,lastname:/^\S+/,email:/^\S+@\S+\.\S+/});function makeIcon(){var name=arguments.length<=0||arguments[0]===undefined?"":arguments[0];return(0,_cycleDom.h)("span.input-group-addon",[(0,_cycleDom.h)("span.glyphicon"+name)])}function makeInputGroup(_ref){var id=_ref.id;var placeholder=_ref.placeholder;var value=_ref.value;var error=_ref.error;var _ref$icoName=_ref.icoName;var icoName=_ref$icoName===undefined?".glyphicon-user":_ref$icoName;return(0,_cycleDom.h)("div.input-group",[makeIcon(icoName),(0,_cycleDom.h)("input"+id+".form-control.input-sm"+error,{type:"text",form:"form",atributes:{placeholder:placeholder},value:value})])}function makeForm(){var _ref2=arguments.length<=0||arguments[0]===undefined?{}:arguments[0];var _ref2$email=_ref2.email;var email=_ref2$email===undefined?"":_ref2$email;var _ref2$firstname=_ref2.firstname;var firstname=_ref2$firstname===undefined?"":_ref2$firstname;var _ref2$lastname=_ref2.lastname;var lastname=_ref2$lastname===undefined?"":_ref2$lastname;var _ref3=arguments.length<=1||arguments[1]===undefined?{}:arguments[1];var _ref3$emailError=_ref3.emailError;var emailError=_ref3$emailError===undefined?false:_ref3$emailError;var _ref3$firstnameError=_ref3.firstnameError;var firstnameError=_ref3$firstnameError===undefined?false:_ref3$firstnameError;var _ref3$lastnameError=_ref3.lastnameError;var lastnameError=_ref3$lastnameError===undefined?false:_ref3$lastnameError;return(0,_cycleDom.h)("form#form",[(0,_cycleDom.h)("feildset",[(0,_cycleDom.h)("legend","Add new contact"),makeInputGroup({id:"#firstname",placeholder:"First name",error:firstnameError?".error":"",value:firstname}),makeInputGroup({id:"#lastname",placeholder:"Last name",error:lastnameError?".error":"",value:lastname}),makeInputGroup({id:"#email",placeholder:"Email",error:emailError?".error":"",icoName:".glyphicon-envelope",value:email}),(0,_cycleDom.h)("button#btn.btn.btn-default.btn-sm",{type:"submit"},[(0,_cycleDom.h)("span.glyphicon.glyphicon-plus-sign"),"  Save"])])])}function intent(DOM){return DOM.get("#form","submit").map(function(e){e.preventDefault();return e.target.elements})}function model(submit$){var adres$=submit$.map(function(_ref4){var email=_ref4.email;var firstname=_ref4.firstname;var lastname=_ref4.lastname;return{email:email.value,firstname:firstname.value,lastname:lastname.value}}).share();var valid$=adres$.filter(formValidator.valid.bind(formValidator)).map(function(adr){adr._id=(0,_cuid2["default"])();return adr});var invalid$=adres$.filter(formValidator.invalid.bind(formValidator));return{valid$:valid$,invalid$:invalid$}}function view(_ref5,edit$){var valid$=_ref5.valid$;var invalid$=_ref5.invalid$;var vtreeSuccess$=valid$.map(function(){return makeForm()});var vtreeFail$=invalid$.map(function(invalidAdr){return makeForm(invalidAdr,formValidator.checkFields.bind(formValidator,invalidAdr)())});var vtreeEdit$=edit$.map(function(item){return makeForm(item)});var vtree$=vtreeSuccess$.merge(vtreeEdit$).merge(vtreeFail$).startWith(makeForm());return vtree$}function adressForm(_ref6){var DOM=_ref6.DOM;var edit$=_ref6.edit$;var action$=intent(DOM);var states=model(action$);var vtree$=view(states,edit$);return{DOM:vtree$,submit$:states.valid$}}exports["default"]=adressForm;module.exports=exports["default"]},{"../form-validator":128,"@cycle/core":1,"@cycle/dom":5,cuid:116}],126:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{"default":obj}}var _cycleCore=require("@cycle/core");var _cycleDom=require("@cycle/dom");var _tableTable=require("./table/table");var _tableTable2=_interopRequireDefault(_tableTable);var _filterInputFilterInput=require("./filter-input/filter-input");var _filterInputFilterInput2=_interopRequireDefault(_filterInputFilterInput);var _adressFormAdressForm=require("./adress-form/adress-form");var _adressFormAdressForm2=_interopRequireDefault(_adressFormAdressForm);var _localStorageSink=require("./local-storage-sink");var _localStorageSink2=_interopRequireDefault(_localStorageSink);var _localStorageSource=require("./local-storage-source");var _formValidator=require("./form-validator");var _formValidator2=_interopRequireDefault(_formValidator);var _view=require("./view");var _view2=_interopRequireDefault(_view);var transformToValidator=function transformToValidator(str){return new _formValidator2["default"]({firstname:RegExp(str),lastname:RegExp(str),email:RegExp(str)})};function model(startList$,delete$,submit$){return startList$.flatMap(function(_ref){var list=_ref.list;var deleteFromList$=delete$.map(function(actAdr){var index=list.findIndex(function(adr){return adr._id===actAdr._id});index!==-1?list.splice(index,1):void 0;return{list:list}});var pushToList$=submit$.map(function(actAdr){list.push(actAdr);return{list:list}});return pushToList$.merge(deleteFromList$)})}function adresses(_ref2){var DOM=_ref2.DOM;var localStorageSink=_ref2.localStorageSink;var localStorageSource=_ref2.localStorageSource;var filter=(0,_filterInputFilterInput2["default"])(DOM);var validator$=filter.input$.startWith("").map(transformToValidator);var startList$=(0,_localStorageSource.deserializeWithDefaults)(localStorageSource).shareReplay(1);var sinkList$=(0,_localStorageSource.deserialize)(localStorageSink);var list$=_cycleCore.Rx.Observable.concat(startList$,sinkList$);var table=(0,_tableTable2["default"])({DOM:DOM,list$:list$,validator$:validator$});var mainForm=(0,_adressFormAdressForm2["default"])({DOM:DOM,edit$:table.edit$});var removeAfterEditing$=table.edit$.sample(mainForm.submit$);var delete$=removeAfterEditing$.merge(table.delete$);var storage$=model(startList$,delete$,mainForm.submit$);var vtree$=(0,_view2["default"])(filter.DOM,mainForm.DOM,table.DOM);return{DOM:vtree$,localStorageSink:(0,_localStorageSink2["default"])(storage$)}}exports["default"]=adresses;module.exports=exports["default"]},{"./adress-form/adress-form":125,"./filter-input/filter-input":127,"./form-validator":128,"./local-storage-sink":129,"./local-storage-source":130,"./table/table":132,"./view":133,"@cycle/core":1,"@cycle/dom":5}],127:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{"default":obj}}var _cycleCore=require("@cycle/core");var _cycleDom=require("@cycle/dom");var _cuid=require("cuid");var _cuid2=_interopRequireDefault(_cuid);function filterInput(DOM){var name=arguments.length<=1||arguments[1]===undefined?"."+(0,_cuid2["default"])():arguments[1];var input$=DOM.get(name+".form-control.input-sm","input").share().map(function(e){return e.target.value}).map(function(str){return str.length>2?str:""}).distinctUntilChanged();var vtree$=_cycleCore.Rx.Observable.just((0,_cycleDom.h)("fieldset",[(0,_cycleDom.h)("legend","Search contact"),(0,_cycleDom.h)("div.input-group",[(0,_cycleDom.h)("span.input-group-addon",[(0,_cycleDom.h)("span.glyphicon.glyphicon-search")]),(0,_cycleDom.h)("input"+name+".form-control.input-sm#search",{atributes:{placeholder:"search"},type:"text"})])]));return{DOM:vtree$,input$:input$}}exports["default"]=filterInput;module.exports=exports["default"]},{"@cycle/core":1,"@cycle/dom":5,cuid:116}],128:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||false;descriptor.configurable=true;if("value"in descriptor)descriptor.writable=true;Object.defineProperty(target,descriptor.key,descriptor)}}return function(Constructor,protoProps,staticProps){if(protoProps)defineProperties(Constructor.prototype,protoProps);if(staticProps)defineProperties(Constructor,staticProps);return Constructor}}();function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor)){throw new TypeError("Cannot call a class as a function")}}var Validator=function(){function Validator(regExps){_classCallCheck(this,Validator);this.regExps=regExps}_createClass(Validator,[{key:"valid",value:function valid(adr){var error=this.checkFields(adr);for(var prop in error){if(error[prop])return false}return true}},{key:"invalid",value:function invalid(adr){return!this.valid(adr)}},{key:"sel",value:function sel(adr){var match=this.checkFields(adr);for(var prop in match){if(!match[prop])return true}return false}},{key:"checkFields",value:function checkFields(adr){var error={};for(var prop in this.regExps){if(!adr[prop]||!this.regExps[prop].test(adr[prop]))error[prop+"Error"]=true;else error[prop+"Error"]=false}return error}}]);return Validator}();exports["default"]=Validator;module.exports=exports["default"]},{}],129:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});exports["default"]=serialize;function serialize(adresse$){return adresse$.map(JSON.stringify)}module.exports=exports["default"]},{}],130:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{"default":obj}}var _ramdaSrcContainsWith=require("ramda/src/containsWith");var _ramdaSrcContainsWith2=_interopRequireDefault(_ramdaSrcContainsWith);var _ramdaSrcAppend=require("ramda/src/append");var _ramdaSrcAppend2=_interopRequireDefault(_ramdaSrcAppend);var addDefaultUniq=function addDefaultUniq(defItem,key,items){return(0,_ramdaSrcContainsWith2["default"])(function(a,b){return a[key]===b[key]},defItem,items)?items:(0,_ramdaSrcAppend2["default"])(defItem,items)};var safeJSONParse=function safeJSONParse(str){return JSON.parse(str)||{list:[]}};var mergeWithDefaultAdresses=function mergeWithDefaultAdresses(adresses){var defaultAdresses=[{email:"key@key.com",firstname:"key",lastname:"key",_id:"test_id1106306"}];return{list:addDefaultUniq(defaultAdresses[0],"_id",adresses.list)}};function deserialize(localStorageValue$){return localStorageValue$.map(safeJSONParse)}function deserializeWithDefaults(localStorageValue$){return localStorageValue$.map(safeJSONParse).map(mergeWithDefaultAdresses)}exports["default"]={deserialize:deserialize,deserializeWithDefaults:deserializeWithDefaults};module.exports=exports["default"]},{"ramda/src/append":117,"ramda/src/containsWith":118}],131:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});var _cycleCore=require("@cycle/core");var _cycleDom=require("@cycle/dom");function intent(DOM,item){return{edit$:DOM.get(".edit."+item._id,"click"),delete$:DOM.get(".delete."+item._id,"click")}}function model(_ref,item$){var edit$=_ref.edit$;var delete$=_ref.delete$;return{editItem$:_cycleCore.Rx.Observable.combineLatest(item$,edit$,function(item){return item}),deleteItem$:_cycleCore.Rx.Observable.combineLatest(item$,delete$,function(item){return item})}}function view(item$){return item$.map(function(item){return(0,_cycleDom.h)("tr."+item._id,{key:item._id},[(0,_cycleDom.h)("td",""+item.firstname),(0,_cycleDom.h)("td",""+item.lastname),(0,_cycleDom.h)("td",""+item.email),(0,_cycleDom.h)("td.text-center",[(0,_cycleDom.h)("span.glyphicon.glyphicon-pencil.edit."+item._id,[(0,_cycleDom.h)("::before")])]),(0,_cycleDom.h)("td.text-center",[(0,_cycleDom.h)("span.glyphicon.glyphicon-remove.delete."+item._id,[(0,_cycleDom.h)("::before")])])])})}function tableItem(DOM){var item=arguments.length<=1||arguments[1]===undefined?{email:"jurstv@gmail.com",firstname:"aas",lastname:"asd",_id:"ce12111"}:arguments[1];var actions=intent(DOM,item);var item$=_cycleCore.Rx.Observable.just(item);var states=model(actions,item$);var vtree$=view(item$);return{DOM:vtree$,edit$:states.editItem$,delete$:states.deleteItem$}}exports["default"]=tableItem;module.exports=exports["default"]},{"@cycle/core":1,"@cycle/dom":5}],132:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{"default":obj}}var _cycleCore=require("@cycle/core");var _tableItem=require("./table-item");var _tableItem2=_interopRequireDefault(_tableItem);function tableConstructor(_ref){var DOM=_ref.DOM;var list$=_ref.list$;var validator$=_ref.validator$;var ti$=_cycleCore.Rx.Observable.combineLatest(list$,validator$,function(_ref2,valid){var list=_ref2.list;return list.filter(valid.sel.bind(valid)).map(function(adress){return(0,_tableItem2["default"])(DOM,adress)})});var vtree$=ti$.map(function(tis){return tis.map(function(ti){return ti.DOM})});var edit$=ti$.flatMap(function(tis){return tis.map(function(ti){return ti.edit$}).reduce(function(acc,cur){return acc.merge(cur)},new _cycleCore.Rx.Subject)}).share();var delete$=ti$.flatMap(function(tis){return tis.map(function(ti){return ti.delete$}).reduce(function(acc,cur){return acc.merge(cur)},new _cycleCore.Rx.Subject)});return{DOM:vtree$,edit$:edit$,delete$:delete$}}exports["default"]=tableConstructor;module.exports=exports["default"]},{"./table-item":131,"@cycle/core":1}],133:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});var _cycleCore=require("@cycle/core");var _cycleDom=require("@cycle/dom");function view(filter$,mainForm$,table$){return _cycleCore.Rx.Observable.combineLatest(filter$,mainForm$,table$,function(filter,mainForm,table){return(0,_cycleDom.h)("div.content",[(0,_cycleDom.h)("div.row",[(0,_cycleDom.h)("div.col-md-5",[(0,_cycleDom.h)("h1","Adress Book")])]),(0,_cycleDom.h)("div.row",[(0,_cycleDom.h)("div.col-md-3",[filter,mainForm]),(0,_cycleDom.h)("div.col-md-9",[(0,_cycleDom.h)("legend","Contact List"),(0,_cycleDom.h)("table.table.table-striped.table-bordered.table-hover.table-condensed",[(0,_cycleDom.h)("tbody#tbody",[(0,_cycleDom.h)("tr",[(0,_cycleDom.h)("th","First Name"),(0,_cycleDom.h)("th","Last Name"),(0,_cycleDom.h)("th","Email"),(0,_cycleDom.h)("th.text-center","Edit"),(0,_cycleDom.h)("th.text-center","Delete")])].concat(table))])])])])})}exports["default"]=view;module.exports=exports["default"]},{"@cycle/core":1,"@cycle/dom":5}],134:[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});var _cycleCore=require("@cycle/core");function makeLocalStorageSourceDriver(keyName){return function(){return _cycleCore.Rx.Observable.just(localStorage.getItem(keyName))}}function makeLocalStorageSinkDriver(keyName){var subject=new _cycleCore.Rx.Subject;return function(keyValue$){keyValue$.subscribe(function(keyValue){localStorage.setItem(keyName,keyValue);subject.onNext(keyValue)});return subject}}exports["default"]={makeLocalStorageSinkDriver:makeLocalStorageSinkDriver,makeLocalStorageSourceDriver:makeLocalStorageSourceDriver};module.exports=exports["default"]},{"@cycle/core":1}]},{},[124]);